...
Sink::Sink(int n, track_t sink_cost)
{
...
void Sink::sink_update_all(std::vector<Node> &V, std::vector<track_t> &distance2src, int sink_id, int n)
{
    for(int i=2; i<n; i+=2){
        auto sink_dist = V[i].successor_edges_weights[0] - distance2src[sink_id];
...
void Sink::sink_update_all_weight(std::vector<Node> &V, std::vector<track_t> &distance2src, int sink_id, int n)
{
    for(int i=2; i<n; i+=2){
        auto sink_dist = V[i].successor_edges_weights[0] - distance2src[sink_id];
        sink_precursor_weights[i] = sink_dist + distance2src[i];
...
void Sink::sink_build_precursormap(std::vector<track_t> &ancestor_ssd, std::vector<int> &ancestor_node_id, std::vector<int> &parent_node_id, int n)
{
...
void Sink::sink_update_all_half(std::vector<track_t> distance2src, int sink_id, int n)
{
    auto sink_dist = distance2src[sink_id];
    for(int i=1; i<n-1; i++){
        auto cur_dist = sink_cost_ + distance2src[i] - sink_dist;
...
void Sink::sink_update_subgraph(std::vector<int> update_node_id, std::vector<track_t> distance2src, int n, int sink_id)
{
    sink_weight_shift = sink_weight_shift - distance2src[sink_id];
    for(auto it = update_node_id.begin(); it != update_node_id.end(); it++){
        int i = *it;
        if ((i)%2 == 0) {
            auto cur_dist = sink_precursor_weights[i] + distance2src[i];
            sink_precursors.insert(std::make_pair(cur_dist, i));
        }
    }
}
