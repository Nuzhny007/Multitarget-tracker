...
public:
    ...
    void Process()
    {
        ...
        while (k != 27)
        {
            ...
            ProcessFrame(gray, framesCounter);
...
protected:
    ...
    virtual void ProcessFrame(cv::Mat grayFrame, int framesCounter) = 0;
...
protected:
    ...
    /// \param grayFrame
    ///
    void ProcessFrame(cv::Mat grayFrame, int framesCounter)
    {
        const std::vector<Point_t>& centers = m_detector->Detect(grayFrame);
        const regions_t& regions = m_detector->GetDetects();

        m_tracker->Update(centers, regions, grayFrame, framesCounter);
    }
